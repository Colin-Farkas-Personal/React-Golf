...
function getLines(element: HTMLElement): Line[] {
        ...
        const pointElement1 = element?.getElementsByClassName("box-collider-point-bl")[0].getBoundingClientRect();
        const pointElement2 = element?.getElementsByClassName("box-collider-point-tl")[0].getBoundingClientRect();
        const pointElement3 = element?.getElementsByClassName("box-collider-point-tr")[0].getBoundingClientRect();
        const pointElement4 = element?.getElementsByClassName("box-collider-point-br")[0].getBoundingClientRect();
...
function isCollisionOnLine(playerBallRadius: number, playerBallCenter: Point, point1: Point, point2: Point) {
    // Calculate vectors
    const v1 = { x: playerBallCenter.x - point1.x, y: playerBallCenter.y - point1.y };
    ...
    const distanceToClosestPoint = Math.sqrt(Math.pow(playerBallCenter.x - closestPoint.x, 2) + Math.pow(playerBallCenter.y - closestPoint.y, 2));
...
export function useBoxCollider(ref: React.RefObject<HTMLElement>) {
    ...
    const playerBallCenter = { x: playerBallRect.left + playerBallRect.width / 2, y: playerBallRect.top + playerBallRect.height / 2 };
    ...
    lines.forEach(line => {
        if (isCollisionOnLine(playerBallRadius, playerBallCenter, line.a, line.b)) {
            ...
            [newVelocity.x, newVelocity.y] = getReflection(line.a, line.b, velocityX, velocityY);
...
function getReflection(point1: Point, point2: Point, velocityX: number, velocityY: number): Velocity {
    ...
    const dotProduct = velocityX * normalizedNormal.x + velocityY * normalizedNormal.y;

    // Calculate reflection vector
    const reflection = { x: velocityX - 2 * dotProduct * normalizedNormal.x, y: velocityY - 2 * dotProduct * normalizedNormal.y };
